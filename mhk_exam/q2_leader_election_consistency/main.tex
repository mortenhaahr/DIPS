...
\begin{frame}
  ...
  \note{
    ...
    Reliability: If we have a replica of the system it might be possible for the system to continue working, even if one of the nodes crash.\\
    Load balancing: If we have replicated systems we can divide the workload of the system between replicated nodes. \\
    Geo scaling: We can place replicas of the overall system close to the user's locations. \\
  }
\end{frame}
...
\begin{itemize}
  ...
  \item Optional: A CA system doesn't distinguish between node failures and network failures. So when a failure occurs it must stop accepting all writes since it cannot know if it is a network partition. It is still somewhat available though as it can process read requests.
  \item Optional: A CP system allows the majority partition to continue operating normally and requires the minority partition to become unavailable.
\end{itemize}}
